...
| Руки за месяц (дистанции) | `distances` | срез: ID клуба + Discord + ID дистанции + рум + месяц + Nitro/Regular + лимит + игровой ник. Пишет Root (`save_distance_dump`). Админ правит в контроле (`save_distance_rows`). Одинаковые срезы не дублируются |
...
| `distances` | Руки за календарный месяц. Срез: карточка клуба (если есть) + Discord + ID дистанции + рум + месяц + вид + лимит + признак + part + ярлык + игровой ник. Пишет Root. Одинаковые не дублируются |

### Дистанции

Одна строка — срез: кто (карточка клуба, если уже есть), Discord ID, ID дистанции, на каком руме, за какой месяц, Nitro или Regular, какой бай-ин, сколько рук.
...
- `entry_kind` по умолчанию `primary`. Корректировка — отдельная строка. Итог = сумма всех признаков.
- `part` по умолчанию `1`. Досыл или смена учёта в середине месяца — `2`, `3`… уже залитый кусок не затираем.
- `batch_label` — ярлык пачки (основная выгрузка / человек дослал позже). Можно оставить тот же или дать новый. В уникальный ключ не входит.
- `note` — комментарий к записи.
...
- Повтор с тем же ключом (рум + месяц + вид + лимит + формат + признак + part + ID дистанции) **не пишется**. Чтобы дописать руки — другой part или `correction`.
- Нет строки — на экране ноль. Год и «всего» не храним, складываем.
- Импорт: Root → «Импорт дистанции», CSV, кнопка «Записать» (`save_distance_dump`, SQL `is_root()`). Тестовую заливку Root стирает кнопкой «Очистить месяц» / «Очистить всю таблицу» (`clear_distance_rows`). Админка «Контроль дистанций» читает уже сохранённые строки и правит руки (`save_distance_rows`, SQL `is_staff()`). Файл на админский экран не попадает. Страница «Дистанция» у игрока — отдельно, позже.
