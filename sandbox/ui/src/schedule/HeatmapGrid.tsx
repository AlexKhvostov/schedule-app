...
function Seat({
  ...
  const empty = !mark;
  const base = empty ? "#ebedf0" : mark.bg;
  return (
    <span
      ...
      style={{
        background: base,
        color: mark?.fg ?? "transparent",
        filter: past ? "brightness(0.58)" : undefined,
      }}
    >
      {mark?.t}
      {now && (
    ...
    <div className="min-h-0 flex-1 overflow-auto px-2 py-4">
      ...
      >
        ...
        {hours.map((h) => (
          <div
            key={h}
            className="text-center font-mono leading-tight tabular-nums text-[#656d76]"
            style={{ gridColumn: "span 2" }}
          >
            <div className="text-[10px] font-semibold">{hourRange(h)}</div>
            <div className="text-[8px] text-[#8aa4d4]">{playerHourRange(h, mskOffset)}</div>
          </div>
        ))}
            ...
            <div key={day.d} className="contents">
              <div
                className={cn(
                  ...
                  today
                    ? "bg-[#ffd966] font-bold text-[#1a1c1e]"
                    : cn("text-[#656d76]", day.weekend && "text-[#9a6700]"),
                )}
                style={{ height: CELL_H }}
              >
              ...
              {halves.map((half) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "flex shrink-0 flex-col overflow-hidden rounded-[3px]",
                      today && "ring-1 ring-[#ffd966]/80",
                    )}
                  ...
                  >
                    {cap === 2 ? (
                      <>
                        <Seat mark={who[0]} past={past} now={now} progress={cet.slotProgress} />
                        <Seat mark={who[1]} past={past} now={now} progress={cet.slotProgress} />
                      </>
                    ) : (
                      <Seat mark={who[0]} past={past} now={now} progress={cet.slotProgress} />
                    )}
