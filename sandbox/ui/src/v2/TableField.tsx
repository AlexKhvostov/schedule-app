...
function nowLineLeft(half: number, progress: number) {
  ...
  return `calc(58px + (100% - 58px) * ${t} / ${SLOT_COUNT})`;
}
...
export function TableField({
  ...
  return (
    <section className="v2-tbl flex min-h-0 flex-1 flex-col overflow-hidden" onMouseLeave={() => setHover(null)}>
      <div className="v2-days min-h-0 flex-1 overflow-auto">
        <div className="v2-days-inner">
          <div className="v2-tbl-hours sticky top-0 z-20">
            <div className="v2-tbl-day v2-tbl-lab">{t("v2.day")}</div>
            <div className="v2-tbl-nl v2-tbl-lab">NL</div>
            <div className="v2-tbl-lv v2-tbl-lab">{t("v2.skin.levelShort")}</div>
            <div className="v2-tbl-track v2-mono relative">
              {hours.map((h) => {
                ...
                return (
                  <span
                    key={h}
                    className={`v2-tbl-hour${on ? " is-nav" : ""}${night ? " is-night" : ""}`}
          ...
          {days.map((day, dayIdx) => {
            ...
            const lanes = limits.flatMap((limit) =>
              lanesOf(capacity, limit, day.d, year, monthIndex).map((level) => ({ limit, level })),
            );
            return (
              <div
                key={day.d}
                className={`v2-tbl-daybox${today ? " is-today" : ""}${rowOn ? " is-nav" : ""}${day.weekend ? " is-weekend" : ""}`}
              >
                <div className="v2-tbl-day v2-mono" style={{ gridRow: `1 / span ${Math.max(1, lanes.length)}` }}>
                  {String(day.d).padStart(2, "0")} {day.wd}
                </div>
                <div className="v2-tbl-body">
                  {today && (
                    <span className="v2-tbl-now" style={{ left: nowLineLeft(cet.half, cet.slotProgress) }} aria-hidden />
                  )}
                  {lanes.map(({ limit, level }) => {
                    const row = grids[limit]?.[dayIdx] ?? [];
                    const limitHours = hoursOf(capacity, limit, day.d, weekdayOf(year, monthIndex, day.d));
                    const laneOn =
                      hover?.dayIdx === dayIdx && hover.limit === limit && hover.level === level;
                    return (
                      <div key={`${limit}-${level}`} className={`v2-tbl-lane${laneOn ? " is-lane" : ""}`}>
                        <div className={`v2-tbl-nl${laneOn ? " is-nav" : ""}`}>
                          {level === 0 ? (
                            <span className="v2-limit-chip" style={{ color: limitTone(limit) }}>
                              {limit}
                            </span>
                          ) : (
                            <span className="v2-tbl-nl-ghost" aria-hidden />
                          )}
                        </div>
                        <div className={`v2-tbl-lv${laneOn ? " is-nav" : ""}`}>{level + 1}</div>
                        <div className="v2-tbl-track">
                          {Array.from({ length: SLOT_COUNT }, (_, half) => {
                            ...
                            const hourEnd = half % 2 === 1;
                            return (
                              <button
                                key={half}
                                type="button"
                                className={`v2-tbl-cell${past ? " is-past" : ""}${locked ? " is-lock" : ""}${mark ? " is-on" : ""}${colOn ? " is-col" : ""}${muted ? " is-dim" : ""}${hit ? " is-hit" : ""}${hourEnd ? " is-hour" : ""}`}
                                disabled={past}
